<template>
  <div v-if="authStore.isExpired" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
    <div class="absolute inset-0 bg-black/20 backdrop-blur-md"></div>

    <div class="relative bg-white rounded-2xl p-8 max-w-sm w-full shadow-2xl border border-black/5 text-center scale-up-center">
...
const relogin = () => {
  // 1. Cerramos el modal en el store
  authStore.setExpired(false)
  
  // 2. Limpiamos datos residuales
  authStore.logout()
...
<style scoped>
/* Un toque de animación para que no aparezca de golpe */
.scale-up-center {
