...
try {
    // Calculamos fechas dinámicas (hoy y hace 7 días)
    ...
    // 2. Asignamos los KPIs (las tarjetas)
    stats.value = {
      totalProductos: res.data.totalProductos,
      valorVentasPeriodo: res.data.ventas,
      inversionComprasPeriodo: res.data.inversion
    }

    // 3. Mapeamos la gráfica (asumiendo que res.data.graficaVentas trae la lista del SP)
    if (res.data.graficaVentas && res.data.graficaVentas.length > 0) {
      ...
      // Actualizamos las fechas del eje X
      chartOptions.value = {
        ...chartOptions.value,
        xaxis: {
          ...chartOptions.value.xaxis,
          categories: res.data.graficaVentas.map(i => formatearFechaParaGrafica(i.fecha))
        }
      };
    }
  } catch (e) {
